...
``interactsh-client`` binary, which is already pinned in ``containers/Dockerfile``.
"""
...
class InteractshProvider(OobProvider):
    ...
    Runs the client in a subprocess, parses the generated hostname from stdout,
    and polls for interactions via the client's JSON output. The public
    ProjectDiscovery server pool is used when ``server_url`` is ``None``;
    a self-hosted server is used when a URL is provided.
    """
    ...
    async def start(self, run_dir: Path) -> None:
        """Start the interactsh-client subprocess and wait for hostname."""
        if self._ready:
            return
        ...
        self._base_host = await self._wait_for_hostname(self._proc.stdout)
        self._ready = True
    ...
    async def _wait_for_hostname(self, stdout: asyncio.StreamReader) -> str:
        ...
        raise RuntimeError("interactsh-client did not emit a hostname within 60s")
    ...
    def _parse_json_line(self, line: str) -> OobHit | None:
        ...
        return OobHit(
            ...
            token=token,
            full_fqdn=full_fqdn,
            source_ip=data.get("remote-address", "0.0.0.0"),
